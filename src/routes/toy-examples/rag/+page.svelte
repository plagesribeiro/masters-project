<script>
  ...
  onMount(() => {
    // Add welcome message specific to RAG
    messages = [{
      id: Date.now(),
      text: $t('ragWelcome'),
      isUser: false,
...
<svelte:head>
  <title>{$t('toyExample2Title')} - {$t('masterDefense')} | Pedro Ribeiro</title>
  <style>
  ...
  <div class="bg-info text-info-content py-3 px-4 shadow-lg flex-shrink-0">
    <div class="max-w-4xl mx-auto flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-lg md:text-xl font-bold">{$t('toyExample2Title')}</h1>
